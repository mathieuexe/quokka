...
import { createTicketMessage, createTicketWithAdminMessage, createTicketWithMessage, getTicketById, getTicketMessages, listAdminTickets, listUserTickets, updateTicket } from "../repositories/ticketRepository.js";
...
const applyTicketRules = (payload, ctx) => {
...
function shouldSetStatusOnAdminReply(currentStatus) {
    if (currentStatus === "Clôturé")
        return null;
    if (currentStatus === "Résolu")
        return null;
    return "En attente utilisateur";
}
function shouldSetStatusOnUserReply(currentStatus) {
    if (currentStatus === "En attente utilisateur")
        return "Ouvert";
    return null;
}
...
export async function postUserTicket(req, res) {
    const userId = req.user?.sub;
    if (!userId) {
        res.status(401).json({ message: "Authentification requise." });
        return;
...
export async function postUserTicketMessage(req, res) {
    const userId = req.user?.sub;
    if (!userId) {
        res.status(401).json({ message: "Authentification requise." });
        return;
...
export async function patchAdminTicket(req, res) {
    const ticketId = req.params.ticketId;
    const ticket = await getTicketById(ticketId);
    if (!ticket) {
        res.status(404).json({ message: "Ticket introuvable." });
...
export async function postAdminTicketMessage(req, res) {
    const adminId = req.user?.sub;
    if (!adminId) {
        res.status(401).json({ message: "Authentification requise." });
        return;
...
export async function postAdminTicketAssign(req, res) {
    const adminId = req.user?.sub;
    if (!adminId) {
        res.status(401).json({ message: "Authentification requise." });
        return;
...
export async function postTicketAttachments(req, res) {
