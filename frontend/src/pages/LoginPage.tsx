...
export function LoginPage(): JSX.Element {
  ...
  const [discordMaintenance, setDiscordMaintenance] = useState(false);
  const [discordMaintenanceMessage, setDiscordMaintenanceMessage] = useState<string | null>(null);
  const discordLoginUrl = `${API_URL}/auth/discord`;

  useEffect(() => {
    let cancelled = false;
    void apiRequest<MaintenanceResponse>("/maintenance")
      .then((data) => {
        if (cancelled) return;
        setDiscordMaintenance(Boolean(data.maintenance.discord_auth_enabled));
        setDiscordMaintenanceMessage(data.maintenance.discord_auth_message || null);
      })
      .catch(() => {
        if (cancelled) return;
        setDiscordMaintenance(false);
        setDiscordMaintenanceMessage(null);
      });
    return () => {
      cancelled = true;
    };
    ...
    <section className="page auth-page">
      <div className="auth-shell">
        ...
        <form className="card form auth-form-card" onSubmit={onSubmit}>
          ...
          {discordMaintenance ? (
            <button className="btn auth-discord-btn" type="button" disabled>
          ...
          {discordMaintenance && (
            <p className="error-text">{discordMaintenanceMessage || "La connexion via Discord est en maintenance."}</p>
          )}
          <label>
            {t("auth.email")}
